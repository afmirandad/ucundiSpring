...
public class camasController {
    ...
    public ResponseEntity<String> readCamas(){
        return ResponseEntity.ok(String.valueOf(camasR.findAll()));
    }

    @GetMapping("/readCamas/{id}")
    public ResponseEntity<Object> readCamas(@PathVariable("id") String identificador){
        String regexforID = "[1-5 a-z]{1,5}";
        if (identificador == null || !identificador.matches(regexforID))
            //throw new IllegalArgumentException();
            return ResponseEntity.notFound().build();
        return ResponseEntity.ok(camasR.findById(Long.valueOf(identificador)));
